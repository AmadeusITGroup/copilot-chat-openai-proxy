...
find test-dist/test/suite -name "*.js" -exec sed -i 's/test-owner/AmadeusITGroup/g; s/test-repo/copilot-chat-openai-proxy/g' {} \; 2>/dev/null || true
...
class Logger {
    constructor() {
        this.outputChannel = vscode.window.createOutputChannel('copilot-chat-openai-proxy');
    }
    static getInstance() {
        if (!Logger.instance) {
